...
export default function GraphWidget({ mode, channel }: WidgetProps) {
  ...
  if (typeof latestValue !== "number") {
    return <div>Values are not numbers and so cannot be displayed on the graph widget.</div>
  }
  ...
  if (mode === "Value") {
    return (
      <div className="w-full h-full flex flex-col justify-center items-center gap-2">
        <p className="font-semibold text-lg">{channel.label}</p>
        <p className="font-normal text-lg">{latestValue.toFixed(3)} {channel.unit || ""}</p>
      </div>
    );
  }
